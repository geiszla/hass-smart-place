...
KNOWN_MESSAGES: Final[list[MessageDefinition]] = [
    ...
    # Parsed so SOUND frames don't fall through to unknown_frames, but
    # intentionally NOT mapped to any state/entity (like ``PackageBox``).
    # The SPA plays /sounds/<lang>/<name>.ogg on this frame; we briefly
    # hypothesised it was the doorbell ring edge, but a live browser test
    # (2026-06-25) showed it's never sent for a ring on this building —
    # the ring is SIP-only. Kept as a known message for a future use.
    MessageDefinition(
        name="Sound",
        description=(
            "Push: play a notification sound. Format: SOUND<n>:<name> "
            "(plays <name>.ogg; 'AUS' = silence). Parsed-but-unused."
        ),
